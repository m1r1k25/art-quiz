...
btnSettings.addEventListener('click', () => {
    blockHomePage.classList.add('hide')
    blockSettings.classList.remove('hide')
})

btnSave.addEventListener('click', () => {
    blockHomePage.classList.remove('hide')
...
const blockCategoryArtists = document.getElementById('blockCategoryArtists')
const blockCategoryPictures = document.getElementById('blockCategoryPictures')
const btnBlockArtists = document.getElementById('btnBlockArtists')
const btnBlockPictures = document.getElementById('btnBlockPictures')
const btnHomeFromArtists = document.getElementById('btnHomeFromArtists')
const btnHomeFromPictures = document.getElementById('btnHomeFromPictures')

btnBlockArtists.addEventListener('click', () => {
    blockHomePage.classList.add('hide')
    blockCategoryArtists.classList.remove('hide')
})

btnBlockPictures.addEventListener('click', () => {
    blockHomePage.classList.add('hide')
    blockCategoryPictures.classList.remove('hide')
})

btnHomeFromArtists.addEventListener('click', () => {
    blockHomePage.classList.remove('hide')
    blockCategoryArtists.classList.add('hide')
})

btnHomeFromPictures.addEventListener('click', () => {
    blockHomePage.classList.remove('hide')
    blockCategoryPictures.classList.add('hide')
})

//----------Choose card------------

const blockGameArtists = document.getElementById('blockGameArtists')
const blockGamePictures = document.getElementById('blockGamePictures')
const logo = document.getElementById('logo')
const card = Array.from(document.querySelectorAll('.card'))
const cardPic = Array.from(document.querySelectorAll('.cardPic'))
const btnHomeFromGameArtists = document.getElementById('btnHomeFromGameArtists')
const btnHomeFromGamePictures = document.getElementById('btnHomeFromGamePictures')

btnHomeFromGameArtists.addEventListener('click', () => {
    blockGameArtists.classList.add('hide')
    blockHomePage.classList.remove('hide')
    logo.classList.remove('hide')
    logo.classList.add('logo')
})

btnHomeFromGamePictures.addEventListener('click', () => {
    blockGamePictures.classList.add('hide')
    blockHomePage.classList.remove('hide')
    logo.classList.remove('hide')
    logo.classList.add('logo')
})
...
card.forEach(item => {
    item.addEventListener('click', () => {
        ...
        blockGameArtists.classList.remove('hide');
        indexOfQuestions = item.dataset['number'] * 10
        arrayOfRightAnswers = [];
        getNewQuestions()
    })
})

const answers = Array.from(document.querySelectorAll('.answers__answer'));
const paginations = Array.from(document.querySelectorAll('.wrapper-game__pagination-item'));
const countRightAnswers = Array.from(document.querySelectorAll('#countRightAnswers'));
const pictureForArtists = document.getElementById('pictureForArtists') 

let indexOfQuestions = 0;
let arrayForQuestions = [];
let arrayAfterRandom = [];
let arrayOfRightAnswers = [];
let arrayOfAuthors = [];

const doNewArrayOfAuthors = () => {
    while (arrayOfAuthors.length) {
        arrayOfAuthors.pop();
    }
    arrayOfAuthors.push(images[indexOfQuestions].author);
    arrayOfAuthors.push(images[Math.floor(Math.random() * 240)].author)
    arrayOfAuthors.push(images[Math.floor(Math.random() * 240)].author)
    arrayOfAuthors.push(images[Math.floor(Math.random() * 240)].author)
}
...
function getNewQuestions() {
    doNewArrayOfAuthors()
    pictureForArtists.style.backgroundImage = `url(img/${indexOfQuestions}.jpg)`;
    arrayAfterRandom = [... arrayOfAuthors.sort(() => Math.random() - 0.5)] // конечный массив 
    answers.forEach(choice => {
        const number = choice.dataset['number'] - 1; //= просто цифра 1, 2, 3, 4
        choice.innerHTML = arrayAfterRandom[number];
    });
    indexOfQuestions++
};
...
answers.forEach(choice => {
    choice.addEventListener('click', e => {
        ...
        if (selectedAnswer.textContent === images[indexOfQuestions - 1].author) { //если выбран правильный ответ
            selectedAnswer.classList.add('correct')
            paginations[(indexOfQuestions - 1) % 10].classList.add('correct')
            arrayOfRightAnswers.push(selectedAnswer);
            audioCorrect.play()
        } else {
            selectedAnswer.classList.add('incorrect')
            paginations[(indexOfQuestions - 1) % 10].classList.add('incorrect')
            audioWrong.play()
        }
        setTimeout(() => {
            ...
            getNewQuestions();
            if ((indexOfQuestions - 1) % 10 === 0 && (indexOfQuestions + 1) !== 0) {
                blockGameArtists.classList.add('hide')
                blockCategoryArtists.classList.remove('hide')
                logo.classList.add('logo')
                card[Math.floor(indexOfQuestions/10 - 1)].classList.remove('filter-gray')
                countRightAnswers[Math.floor(indexOfQuestions/10 - 1)].innerHTML = arrayOfRightAnswers.length;
                audioEnd.play()
...
cardPic.forEach(item => {
    item.addEventListener('click', () => {
        paginationsPictures.forEach(item => {
            item.classList.remove('correct-picture')
            item.classList.remove('incorrect-picture')
        })
        blockCategoryPictures.classList.add('hide')
        logo.classList.remove('logo')
        logo.classList.add('hide')
        blockGamePictures.classList.remove('hide')
        indexOfQuestions = item.dataset['number'] * 10
        getNewQuestionsPictures()
    })
})

const answersPicture = Array.from(document.querySelectorAll('.wrapper-game__pictures'));
const paginationsPictures = Array.from(document.querySelectorAll('.wrapper-game__pagination-itemPictures'));
const countRightAnswersPictures = Array.from(document.querySelectorAll('#countRightAnswersPictures'));
const author = document.getElementById('author')

let arrayForQuestionsPictures = [];
let arrayAfterRandomPictures = [];
let arrayOfRightAnswersPictures = [];
let arrayOfPictures = [];

function getNewQuestionsPictures() {
    doNewArrayOfPictures()
    author.innerHTML = images[indexOfQuestions].author;
    arrayAfterRandomPictures = [... arrayOfPictures.sort(() => Math.random() - 0.5)] // конечный массив 
    answersPicture.forEach(item => {
        const number = item.dataset['number'] - 1; //= просто цифра 1, 2, 3, 4
        item.style.backgroundImage = arrayAfterRandomPictures[number];
    });
    indexOfQuestions++
};

const doNewArrayOfPictures = () => {
    while (arrayOfPictures.length) {
        arrayOfPictures.pop();
    }
    arrayOfPictures.push(`url(img/${indexOfQuestions}.jpg)`)
    arrayOfPictures.push(`url(img/${Math.floor(Math.random() * 240)}.jpg)`)
    arrayOfPictures.push(`url(img/${Math.floor(Math.random() * 240)}.jpg)`)
    arrayOfPictures.push(`url(img/${Math.floor(Math.random() * 240)}.jpg)`)
} 

answersPicture.forEach(choice => {
    choice.addEventListener('click', e => {
        const selectedAnswer = e.target;       
        if (selectedAnswer.style.backgroundImage == `url("img/${indexOfQuestions - 1}.jpg")`) { //если выбран правильный ответ
            selectedAnswer.classList.add('correct-picture')
            paginationsPictures[(indexOfQuestions - 1) % 10].classList.add('correct')
            arrayOfRightAnswersPictures.push(selectedAnswer);
            audioCorrect.play()
        } else {
            selectedAnswer.classList.add('incorrect-picture')
            paginationsPictures[(indexOfQuestions - 1) % 10].classList.add('incorrect')
            audioWrong.play()
        }
        setTimeout(() => {
            selectedAnswer.classList.remove('correct-picture')
            selectedAnswer.classList.remove('incorrect-picture')
            getNewQuestionsPictures();
            if ((indexOfQuestions - 1) % 10 === 0 && (indexOfQuestions + 1) !== 0) {
                blockGamePictures.classList.add('hide')
                blockCategoryPictures.classList.remove('hide')
                logo.classList.add('logo')
                cardPic[Math.floor((indexOfQuestions-120)/10 - 1)].classList.remove('filter-grayPictures')
                countRightAnswersPictures[Math.floor((indexOfQuestions-120)/10 - 1)].innerHTML = arrayOfRightAnswersPictures.length;
                audioEnd.play()
                paginationsPictures.forEach(element => {
                    element.classList.remove('incorrect')
                    element.classList.remove('correct')
                });
            }
...
const volumeOnPic = document.querySelector('#mute')

let audioMute = 'true'

function mute() {	
    if (audioCorrect.muted && audioWrong && audioEnd.muted) {		
        audioCorrect.muted = false; 
        audioWrong.muted = false; 
        audioEnd.muted = false;	
        audioMute = false
        volumeOnPic.src = 'images/volume.png';
    } else {		
        audioCorrect.muted = true; 
        audioWrong.muted = true; 
        audioEnd.muted = true;
        audioMute = true			
        volumeOnPic.src = 'images/volume-off.png';
    }
}

volumeOnPic.addEventListener('click', mute)

const rng = document.getElementById('range');

function moveSound() {
    audioCorrect.volume = rng.value / 100;
    audioWrong.volume = rng.value / 100;
    audioEnd.volume = rng.value / 100;
}

rng.addEventListener('mousemove', moveSound);

function setLocalStorage() {
    window.localStorage.setItem('sound', audioMute);
    window.localStorage.setItem('soundVol', volumeOnPic.src);
    window.localStorage.setItem('volume', audioCorrect.volume)
}

window.addEventListener('beforeunload', setLocalStorage);

function getLocalStorage() {
    audioMute = window.localStorage.getItem('sound')
    if (audioMute === 'false') {
        audioCorrect.muted = false;
        audioWrong.muted = false;
        audioEnd.muted = false;
    ...
    volumeOnPic.src = window.localStorage.getItem('soundVol')
    audioCorrect.volume = window.localStorage.getItem('volume')
    audioWrong.volume = window.localStorage.getItem('volume')
    audioEnd.volume = window.localStorage.getItem('volume')
    rng.value = window.localStorage.getItem('volume') * 100
}
